...
const MintingSubmit = (props: MintingSubmitProps) => {
  ...
  useEffect(() => {
    ...
    } else setMintButtonDisable(true);
    ...
    <div className="min-block last ">
      <div className="volume-box">
        {props.priceCap.map((item, index) => (
          <button
            className={`volume-items item0${index + 1} ${
              selectedAmount === item.price ? "active" : ""
            }`}
      ...
      <div className="block-title">{t(`minting.amount`)}</div>
      ...
      <div className="total-box">
        <div className="block-title">{t(`minting.price`)}</div>
        <div className="total-price">
          <span>$</span>
          <span className="price">{addComma(amount)}</span>
        </div>
      </div>
