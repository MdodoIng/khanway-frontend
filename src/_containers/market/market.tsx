...
const MarketContainer = () => {
  useTranslation();
  useStylesheet("/css/market_page.css");
  ...
  return (
    <div id="main-wrapper" className="">
      ...
      <div className="container m-items">
        <div className="row">
          <div
            className="col-xl-12 nfwitem-title aos-init aos-animate"
            data-aos="fade-up"
          >
            <h1 className="text-line-deco">Top NFW items</h1>
          </div>
          <div className="mynft-area col-xl-12">
            {data?.getMainNFTList.onSaleNFTList?.map((item, index) => (
              <NFWItem item={item} key={index} />
            ))}
          </div>
          <Link
            to={"/market/explore"}
